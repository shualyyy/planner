...
function StatusIcon({ status }: { status: TaskStatus }) {
  ...
  if (status === 'not_started') return <span style={{ ...base, border: '2px solid var(--border-2)' }} />
  if (status === 'in_progress') return <span style={{ ...base, border: '2px solid var(--accent)', background: `linear-gradient(90deg,var(--accent) 50%,transparent 50%)` }} />
  if (status === 'blocked') return <span style={{ ...base, border: '2px solid var(--danger)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ width: 6, height: 1.5, background: 'var(--danger)' }} /></span>
  return <span style={{ ...base, background: 'var(--success)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="var(--bg)" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg></span>
}
...
function ProjectDetailView({ project, onBack, onAddTask }: {
  ...
  const metrics = [
    { value: total,                      label: 'Total',   color: 'var(--text)' },
    { value: byStatus.in_progress.length, label: 'Active',  color: 'var(--accent)' },
    { value: byStatus.blocked.length,    label: 'Blocked', color: 'var(--danger)' },
    { value: `${pct}%`,                  label: 'Done',    color: 'var(--success)' },
  ]
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg)', display: 'flex', flexDirection: 'column', animation: 'slideUp 0.25s ease' }}>
      ...
      <div style={{ padding: '8px 22px 0', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 12 }}>
          ...
          <h1 style={{ font: '300 30px/1 var(--font-sans)', color: 'var(--text)' }}>{project.name}</h1>
        </div>
      </div>
      ...
      <div style={{ padding: '16px 22px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
          {metrics.map((m, i) => (
            <div key={i} style={{ flex: 1, padding: '12px 8px', textAlign: 'center', borderRight: i < 3 ? '1px solid var(--border)' : 'none' }}>
              <div style={{ font: '600 18px/1.2 var(--font-sans)', color: m.color }}>{m.value}</div>
              <div style={{ font: '500 9px/1.2 var(--font-sans)', letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--text-faint)', marginTop: 3 }}>{m.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '18px 22px 12px', flexShrink: 0 }}>
        <span style={{ font: '600 10px/1.2 var(--font-sans)', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--accent)' }}>Board</span>
      </div>
              ...
              return (
                <div key={st} style={{ width: 210, flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 11 }}>
                    ...
                    <span style={{ font: '600 12px/1.2 var(--font-sans)', color: 'var(--text)' }}>{TASK_STATUSES[st].name}</span>
      ...
      {/* ── Members bottom sheet — z-index 500 (above tab bar 400) ── */}
      {showMembersSheet && (
        <div
        ...
        >
          <div
            ...
            style={{ width: '100%', background: 'var(--surface)', borderRadius: '24px 24px 0 0', padding: '0 0 max(env(safe-area-inset-bottom,0px),24px)', boxShadow: 'var(--sheet-shadow)', animation: 'slideUp 0.22s ease' }}
          >
            {/* Handle */}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '4px 20px 14px' }}>
              ...
              <button
                ...
                style={{ display: showInvite ? 'none' : 'flex', alignItems: 'center', gap: 5, height: 32, padding: '0 14px', borderRadius: 999, background: 'var(--accent-soft)', border: 'none', cursor: 'pointer', color: 'var(--accent)', font: '600 12px/1 var(--font-sans)' }}
              >
            ...
            {/* Member list */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '0 20px', maxHeight: 280, overflowY: 'auto' }}>
              {profile && (
      ...
      <button
        ...
        style={{
          ...
          boxShadow: '0 1px 0 rgba(255,255,255,0.15) inset, 0 8px 24px var(--accent-glow)',
...
function ProjectCard({ project, stats, onTap }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ font: '600 15px/1.2 var(--font-sans)', color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{project.name}</div>
        <div style={{ font: '400 11px/1.2 var(--font-sans)', color: 'var(--text-muted)', marginTop: 4 }}>{stats.total} task{stats.total !== 1 ? 's' : ''}</div>
      </div>

      <div style={{ flexShrink: 0 }}>
        <div style={{ width: 80, height: 3, borderRadius: 999, background: 'var(--surface2)', overflow: 'hidden' }}>
          <div style={{ width: `${pct}%`, height: '100%', background: project.color, borderRadius: 999 }} />
        </div>
        <div style={{ font: '600 10px/1.2 var(--font-sans)', color: 'var(--text-muted)', marginTop: 5 }}>{pct}%</div>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
        {[
          { c: 'var(--accent)', n: stats.inProgress },
          { c: 'var(--danger)', n: stats.blocked },
          { c: 'var(--success)', n: stats.done },
        ].map((row, i) => (
          <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <span style={{ width: 7, height: 7, borderRadius: 2, background: row.c }} />
            <span style={{ font: '500 10px/1.2 var(--font-sans)', color: 'var(--text-muted)' }}>{row.n}</span>
          </span>
        ))}
      </div>

      <span style={{ font: '300 20px/1.2 var(--font-sans)', color: 'var(--text-faint)', flexShrink: 0 }}>›</span>
    </button>
  )
}
      ...
      <div style={{ padding: '8px 22px 14px', flexShrink: 0 }}>
        {searchOpen ? (
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
              <div style={{ font: '300 34px/1 var(--font-sans)', color: 'var(--text)', letterSpacing: '-0.01em' }}>Projects</div>
              <div style={{ font: '400 12px/1.2 var(--font-sans)', color: 'var(--text-muted)', marginTop: 7 }}>{visibleProjects.length} project{visibleProjects.length !== 1 ? 's' : ''} · {totalTasks} task{totalTasks !== 1 ? 's' : ''}</div>
            </div>
            <button
              onClick={() => setSearchOpen(true)}
              style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}
              title="Search"
            >
